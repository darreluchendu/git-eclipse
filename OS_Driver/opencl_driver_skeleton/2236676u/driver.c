...
CLObject* init_driver() {
    ...
    size_t global;                      // global domain sizeof(int) * buffer_size for our calculation
    size_t local;                       // local domain sizeof(int) * buffer_size for our calculation
    ...
    // get sizeof(int) * buffer_size of kernel source
...
int run_driver(CLObject* ocl,unsigned int buffer_size,  int* input_buffer_1, int* input_buffer_2, int w1, int w2,int* output_buffer) {
    long long unsigned int tid = ocl->thread_num;
     ...
     // Get the maximum work group sizeof(int) * buffer_size for executing the kernel on the device
...
for (int max=0; max<max_iters; max++){
    input1= clCreateBuffer (ocl->context,CL_MEM_READ_WRITE,sizeof(int) * buffer_size,NULL, &err);
     if (err != CL_SUCCESS) {
         fprintf(stderr,"Error: Failed to Create buffer %d\n", err);
         exit(EXIT_FAILURE);
     }

    input2= clCreateBuffer (ocl->context,CL_MEM_READ_WRITE,sizeof(int) * buffer_size,NULL, &err);
     if (err != CL_SUCCESS) {
         fprintf(stderr,"Error: Failed to Create buffer %d\n", err);
         exit(EXIT_FAILURE);
     }

    output= clCreateBuffer (ocl->context,CL_MEM_READ_WRITE,sizeof(int) * buffer_size,NULL, &err);
     if (err != CL_SUCCESS) {
         fprintf(stderr,"Error: Failed to Create buffer %d\n", err);
         exit(EXIT_FAILURE);
     }

    status_buf= clCreateBuffer (ocl->context,CL_MEM_READ_WRITE,sizeof(int) * buffer_size,NULL, &err);
     if (err != CL_SUCCESS) {
         fprintf(stderr,"Error: Failed to Create buffer %d\n", err);
         exit(EXIT_FAILURE);
     }
...
//    c
    ...
    if (err != CL_SUCCESS) {
         fprintf(stderr,"Error: Failed to set arguments %d\n", err);
         exit(EXIT_FAILURE);
    }
    ...
    if (err != CL_SUCCESS) {
         fprintf(stderr,"Error: Failed to set range %d\n", err);
         exit(EXIT_FAILURE);
    }
    ...
    if (err != CL_SUCCESS) {
        fprintf(stderr,"Error: Failed to send finish interrupt %d\n", err);
        exit(EXIT_FAILURE);
    }
    // Check the status
    ...
    if (err != CL_SUCCESS) {
         fprintf(stderr,"Error: Failed to read status %d\n", err);
         exit(EXIT_FAILURE);
    }
    ...
    if (status[0]==0){
           ...
           if (err != CL_SUCCESS) {
               fprintf(stderr,"Error: Failed to read output %d\n", err);
               exit(EXIT_FAILURE);
           }
           break;
       }
    ...
    err=clReleaseMemObject (input1);
      if (err != CL_SUCCESS) {
          fprintf(stderr,"Error: Failed to release mem object %d\n", err);
          exit(EXIT_FAILURE);
      }
